...
- **P2 生 hex の散布**：`ui/*.kt`（`MagiTokens.kt` 除く）に新規の `Color(0x……)` 直書きを増やさない。
  既存の WCAG 補正値・凡例色は既知の baseline として許容。**3.409.5 でラチェット化**＝`P2_BASELINE` を
  超えたら lint が exit 1（旧: 報告するだけで増えても通っていた＝「増分を監視」が実態でなかった）。
  減ったときも定数の更新を促して落とす（下げ忘れると次の増加を見逃すため）。
...
- **P4 任意角丸**：カード/チップ級（**≥8dp**）の新規任意角丸禁止（`MaterialTheme.shapes.*` を使う）。
  ...
  **現状の baseline は 12 件**（16dp×2・8dp×10。2026-08-19 実測。旧記述「残 6 件」は stale だった）。
  非同値（16→14・8→10 の微差）で device 確認しつつ寄せる backlog。P2 と同じくラチェット。
