...
object PolishGate {
     ...
     * 解ける可能性がある局面）の停滞閾値 `stallMs = searchWindowMs * この割合` の割合。既定 **0.9**
     * ＝現行の固定値 `9/10` と厳密に同一（byte-identical・非破壊）。
     *
     ...
     * `stallMs = 0.9 × searchWindowMs` に対し、上書きが `searchWindowMs` 以内に収まるには
     * `factor < 1/0.9 ≈ 1.111` が必要。しかし factor が 1.0 に近いほど上書きは基準閾値と重複するだけ
     * （上書きは本来「フェーズ猶予が満たせない場合の保険」＝基準より緩い＝より長く待つ側であるべき）で、
     * factor が 1.111 に近いほど上書き閾値は `≈0.999×searchWindowMs`（探索締切と実質同じ）＝早期終了の
     ...
     * を早すぎるタイミングで諦め、実機ログで残り250sを無駄にした（3.322-324行のコメント参照）。
