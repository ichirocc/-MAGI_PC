...
public static partial class V6HotfixPasses
{
    ...
    /// は、X追加が low/high(90/45、当時の重み)&gt;c1(30×窓数)で必ず isBetter に棄却され、**i自身のXを
    /// 余剰位置→不足窓へ移す回数保存の移設**だけが唯一の改善手と判明（行内2日swapの貪欲シムで c1 115→62,
    /// -46%）。[2026-09-10/HF77明示指示] high は 45→25 で c1(30) より軽くなったため、この不等式（X追加が
    /// 必ず棄却される）はもう成り立たない局面がある＝1窓だけの不足なら X追加が isBetter を通ることがある。
    /// 手R1/手R2 自体のロジック・安全性（keep-best）は不変＝isBetter が選ぶ手が変わるだけ。
        ...
        bool combineExhaustPairs = false)
    {
        ...
        while (pass < maxPasses)
        {
            ...
            //   c1違反セルが同じセルでc3n(HARD,重み9000)等の更に重い違反も起こしている場合、そのセルの
