...
public static class Ws1Ops
{
    ...
    /// <summary>
    /// [レビュー指摘 2026-09-04] <c>GroupShiftApt</c> を G×K に揃える（読込時の正規化）。空配列・行不足は空欄＝目標なしで埋め、
    /// 余分な列は落とす。列不足の行も同じ規則で埋めるが、読込経路では先に <c>Validate</c> が拒否するため実際に通るのは
    /// 空配列・行不足だけ（Kotlin 3.491.0 の注記と同じ契約）。既に G×K なら同じインスタンスを返す。
