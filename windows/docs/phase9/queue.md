...
| 15 | 編集 | 月次チェックリスト（職員/希望/必要人数/診断→つくる） | `MagiSetupCards.kt` `MonthlyChecklistCard` | `EditView` | 済 | 済 | 未 | `EditView.RenderChecklist`（月次条件の「次の一手」直下。希望行→希望欄へスクロール、診断行→その場で上位6件）。VM に `MonthlyChecklist()`（テスト 2 件）。「つくる」ボタンは原本 3.482.0 で撤去済み＝置かない |
