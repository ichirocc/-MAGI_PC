...
| 18 | 分析 | 要確認一覧＋日別/人別の注意リスト＋内訳（統合カード, 3.459.0/3.471.0） | `MagiDashboardCards.kt` `ViolationHubCard`/`ConfirmListBody`/`AttentionBody`/`BreakdownBody`, `AnalysisTriage.kt` | `AnalysisView` | 済 | 済 | 未 | 原本は 3.471.0 で `AnalysisTriageCard`＋`AnalysisTriage.kt` に置き換わっている（queue の名前は 3.459.0 時点）。`MagiApp.ViewModels/AnalysisTriage.cs`（純関数・テスト 8 件）＋`UiState.EngineRan`＋`AnalysisView.RenderTriage`（サマリー節の上）。場所の列挙は既存「違反の場所」節が担う |
