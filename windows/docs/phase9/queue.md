...
| 17 | 編集 | 制約10族の「詳しい説明」ⓘ展開（3.409.14） | `ConstraintHelp.kt`, `ConstraintEditor.kt` `ConstraintHelpExpander` | `EditView` | 済 | 済 | 未 | `MagiApp.ViewModels/ConstraintHelp.cs`（本文は原本と同一）＋ `ConstraintHelpTest`（10族キーと過不足なし・重み数値なし）。`EditView.RenderConstraintHelp`＝「登録済みのルール」直上の既定閉じリンク。締めの文が指す「直す優先順位」の表は #22 で実在化 |
