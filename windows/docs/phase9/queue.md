...
| 16 | 編集 | 実働チェック（シフト別 担当人数・需要・欠勤余裕） | `MagiSetupCards.kt` `StaffingRealityCard` | `EditView` | 済 | 済 | 未 | `EditView.RenderStaffingReality`（年間マスターの見直し候補メモ直後＝Android と同じ位置）。VM に `StaffingReality()`（テスト 2 件、日別例外込みの需要を確認） |
