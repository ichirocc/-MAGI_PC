...
| 14 | 編集 | 希望シフトのカレンダー（複数日一括・移動の意味論） | `WishEditor.kt` `WishMonthGrid`/`WishApplyPanel` | `EditView` | 済 | 済 | 緑 | run 34012153647（bac5495 は `IReadOnlyList<string>.IndexOf` で CS1929 → 3b5b461 で手走査に修正、1回目で緑）。既存の希望カレンダーに差分（登録済みをシフト色チップ・選択✓・「N日選択中: 日付」・担当可能シフトの大ボタン＋その他⚠・キャンセル）。`RenderWishShiftButtons` ||
| 15 | 編集 | 月次チェックリスト（職員/希望/必要人数/診断→つくる） | `MagiSetupCards.kt` `MonthlyChecklistCard` | `EditView` | 済 | 済 | 緑 | run 34012437511。`EditView.RenderChecklist`（月次条件の「次の一手」直下。希望行→希望欄へスクロール、診断行→その場で上位6件）。VM に `MonthlyChecklist()`（テスト 2 件）。「つくる」ボタンは原本 3.482.0 で撤去済み＝置かない |
| 16 | 編集 | 実働チェック（シフト別 担当人数・需要・欠勤余裕） | `MagiSetupCards.kt` `StaffingRealityCard` | `EditView` | 済 | 済 | 緑 | run 34012656951。`EditView.RenderStaffingReality`（年間マスターの見直し候補メモ直後＝Android と同じ位置）。VM に `StaffingReality()`（テスト 2 件、日別例外込みの需要を確認） |
| 17 | 編集 | 制約10族の「詳しい説明」ⓘ展開（3.409.14） | `ConstraintHelp.kt`, `ConstraintEditor.kt` `ConstraintHelpExpander` | `EditView` | 済 | 済 | 緑 | run 34015238530。`MagiApp.ViewModels/ConstraintHelp.cs`（本文は原本と同一）＋ `ConstraintHelpTest`（10族キーと過不足なし・重み数値なし）。`EditView.RenderConstraintHelp`＝「登録済みのルール」直上の既定閉じリンク。締めの文が指す「直す優先順位」の表は #22 で実在化 |
| 18 | 分析 | 要確認一覧＋日別/人別の注意リスト＋内訳（統合カード, 3.459.0/3.471.0） | `MagiDashboardCards.kt` `ViolationHubCard`/`ConfirmListBody`/`AttentionBody`/`BreakdownBody`, `AnalysisTriage.kt` | `AnalysisView` | 済 | 済 | 緑 | run 34015682978。原本は 3.471.0 で `AnalysisTriageCard`＋`AnalysisTriage.kt` に置き換わっている（queue の名前は 3.459.0 時点）。`MagiApp.ViewModels/AnalysisTriage.cs`（純関数・テスト 8 件）＋`UiState.EngineRan`＋`AnalysisView.RenderTriage`（サマリー節の上）。場所の列挙は既存「違反の場所」節が担う |
...
| 20 | 分析 | C1 頭打ち診断・回数固定の影響（表示の突き合わせ） | `MagiDashboardCards.kt` `C1PlateauCard`/`PinFixedImpactCard` | `AnalysisView` | 済 | 済 | 未 | `AnalysisView.RenderC1Plateau`（新設: 原因未確定／上位6件＋原因チップ＋推奨手・既定で畳む内訳）＋`RenderPinTargets` 改修（緩め幅を下限側/上限側に分離、上位5件、注記を畳む）。VM 変更なし |
