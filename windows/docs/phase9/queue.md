...
| 10 | 勤務表 | シフト別不足サマリーの1行バナー（3.116.0） | `MagiScheduleViews.kt` `ScheduleGrid` 冒頭 | `ScheduleView` | 済 | 済 | 未 | `ScheduleView.RenderShortageBanner`（needViolations の covU 集計）。日ヘッダ「▼N」（Ui.V6.DayRisks）も同時に配線＝#9 の気づきを解消 |
