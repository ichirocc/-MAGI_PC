...
- 2026-09-25（画面層の精読 #2 の C# 移植、Kotlin c66a834〈`claude/merge-wub4fq`・main 未マージ〉同日）: Android の検証済み指摘のうち C# に
  ...
  **VM**: vm-1 盤面を差し替えるジョブ（`BeginBoardJob`・`RunInBackground`）の開始で直し方の探索を捨てる（`CancelFixSearch`）＋ジョブ中に
  終わった探索は書き戻さない。vm-2 背景結果を反映した後の失敗は「失敗しました」でなく前景 §10 と同じ文言で今の盤面を描き直す（C# は落ちない）。
  vm-3 読込・下書きは差し替え後の診断が中止/失敗したら元へ戻す（読込の undo 消去とヒント記憶のリセットは表示の成功後へ）。`ImportCsvCoreAsync` に
  Android 3.592.0 の巻き戻しが未移植だった＝`RollbackBoardCommit` を足した（外した段の「他の案」も戻す）。vm-4 `RunSummary` を PushUndo・元に戻す・
  やり直し・最適化/仕上げの開始・読込で、`CopilotHint` を読込で消す＋読込で `_lastSettingsSig`/`_lastResultHard`/`_lastTopHardFamily` を
  リセット（Kotlin 925-930 の未移植）。vm-5 読込で他の案・直し方・下限判定を消す。vm-6 通常の実行の失敗も入力の盤面で数え直し `HasResult` は
  実行前の値へ。vm-7 停止で `EngineRan` を実行前の値へ。背景結果の採用/維持で `_lastTopHardFamily` も更新。vmio-5 チップ件数を `NeedFamilies`
  から数える。vmio-6 職員一覧の取込は別形式のCSVを取込の前に断る（`StaffCsvIO.OtherKindOf` 新設＋テスト）。vmio-7 案内先は **C# の画面の
  ボタン名**（設定タブ『データ（種類別のCSV）』の『…CSVを書き出す』、名簿は『名簿CSVを新規データとして取り込む』。Android の「取込種別」
  「コンポーネント別 出力」は C# に無い）。vmio-8 古い重みの数字をコメントから外した（`Editing`・`UiState`・`ViolationChecker`・HF66/HF67/Range）。
  **dashboard**: 1 一括クリアの件数を `WishOutOfScopeCount()` から。3 他の案は適用後も一覧を残し `AlternativeApplied` を太字「適用中」、
  undo の段（`UndoSnap.Alts`）に有効だった一覧を退避して元に戻す/やり直す/巻き戻しで盤面と一緒に戻す。案が長く残るようになったので、
  C# に未移植だった Android 3.529.0 の指紋照合（`_altBoardKey`/`_altStateKey`＝計算後に盤面/設定が変わった案は適用しない）と 3.475.0 の
  再検査の世代（案1→案2 の連打で先の報告が上書きしない）も入れた。5 ホームの自動探索は職員を絞った結果（探索中も）を全体探索へ差し替え、
  「探して0件」の盤面（`FixSearched`）では探し直さない。8 コパイロットの「手修正」は勤務表タブ、取りこぼし分の「希望シフトを編集」は
  編集タブを月次条件で開く（`MainWindow.OpenEditDoor`）。9 「担当できるシフトを増やす」の行き先を年間マスターへ（C# に ① の番号は無い）。
  10 `AnalysisTriage.HomeRemainingLabel`（要調整は件数の族だけ、pt だけなら「残りは偏りのみ」）。11 `GetAlgorithmLabel` の名前（と未同期だった
  3.551.0 の説明文）を Kotlin と一致＋テスト。12 チップの c1 を「期間の制約」、群ルールを「グループルール」。13 「次の一手」は画面下のボタン名。
  15 `RelaxStaffRangePin` は lo==hi の行だけ（連打で2段広げない）。
  **schedule**: 2 上限人数の見出しに「(2パターン時)」（適用パネル・日別の例外一覧・年間マスター）、2パターン目 OFF で上限だけなら適用しない。
  5 右端まで送ったら最終週（`ScheduleNav.CurrentWeekIndex`）・送れないときは「次週」を無効。6 全員への一括の希望は担当できる職員だけ。
  11 同じ見直し候補メモは積まない。取りこぼし分（C# のみ）: 勤務表の希望の丸は担当できる希望だけ（チェッカーの pref と同じ）。
  **editors**: 2 1つだけの並びは受け付けたまま「X（1つだけ）」と出し、禁止の並びは全日禁止の注記（一覧と追加/変更後のヒント）。途中に空欄の
  並びは入力検査と VM の両方で断る（C# は 1番目が空でも通って黙って捨てていた）。3 シフトの改名で表示色のキーを付け替え・削除で取り除く
  （`Ws1Ops`、テスト 4 件を移植。状態の等価のテストは C# の record がリストを参照で比べるので値の比較に置換）。4 個人の合計は 1 以上、
  グループ/スキルグループのレンジは両方空欄を断る（期間の制約は C# が既に厳しい）。5 表示色は `ApplyDisplayOnly`（他の案・直し方を消さず
  検査も回さない、続けての色変更は1つの undo＝`UndoSnap.DisplayEdit`）、何も変えないシフト編集・同じ日数の変更は何もしない（値で比較）。
  7 期間を縮めるとき確認を挟む。取りこぼし分: 全員が個人設定済みのグループ単位の回数は何も書かず知らせる（C# の文言は『解除』ボタン）。
  11 グループ単位の回数の説明を D9 に合わせた。14 画面の語を「グループ／スキルグループ」へ（`Problem` の未解決行・設定の見直し・制約の見出しと
  入力欄・ヘルプ・EditView/AnalysisView の見出しと内訳名。CSV の種別タグとログは据え置き）。15 同じ条件の行は追加/変更で断る（数値は 05=5）。
  並びは Android 3.482.0 の族をまたぐ重複判定も C# に無かった＝同時に移植。既存の重複は消さない（エンジンも dedup しない）。16 入力ガイドの
  ルール件数に cons41s/cons42s/cons3w を足した。
  **当たらない**: vm の「背景投入直後のやめる」（C# は `SetRunning(true)` が先＝隙間が無い）・dashboard-2/4/6/14・schedule-1/3/4/7/8/9/10・
  editors-6/8/9/10(チップ)/12/13（C# に該当画面が無い・既に正しい・デスクトップ）。editors-1 は見出しが無いのでヘルプの一文だけ同期。
  docs は `data-models.md`・`sudo_model.md` の該当行を Android と揃えた。テスト: `MagiApp.ViewModels.Tests` 476→515、`MagiEngine.Tests` 941→947
  （`Ws1OpsShiftColorTest` 4・`StaffImportRecognisesOtherComponentHeaders`・`AlgorithmLabelNamesUseChipVocabulary`）。重み・探索は不変。
...
- 2026-09-25（独立レビュー: 編集タブ「職員管理」のスキル区分コンボ、C# のみ＝出力不変）: ①「追加」がコンボの選択（＝直前に見ていた職員の
  スキル区分）を新しい職員へ書いていた。Android の `addStaff` は区分を選ばない（`Staff` の既定値のまま）＝書かないように揃えた
  （コンボは選択中の職員の値を映すだけ）。②保存値が範囲外（区分が無いのに 0 など）だとコンボが前の職員の選択のまま残り、「改名・所属変更」が
  それを書き戻していた＝`MagiViewModel.StaffSkillComboIndex`（-1・範囲外は「(なし)」、Android `StaffManageCard` と同じ読み）で常にその職員の
  値を映し、`SetStaffSkillFromCombo` は取り込んだ位置からコンボが動いたときだけ書く（触っていない範囲外の 0 を黙って書き換えない）。区分の増減で
  番号がずれたら取り込み直す。③コメント「既定0=未所属」は誤り（0＝先頭の区分、-1＝未所属）＝①の書き換えで削除。`Staff.SkillIdx` の既定値は
  backlog #38 の判断待ちで触らない。テスト 3 件（`MagiViewModelWs1Test`）。盤面・評価は不変。
...
- 2026-09-25（既定OFF台帳の整理を同期、Kotlin 同日）: Android が測定で否決済みの既定OFF分岐 8 件を撤去（出力不変、決定的後処理の盤面ハッシュ 8/8 一致）。
  ...
  `debtLaneSlots`・`SaParams.officialTieBreak`）は C# へ移植していなかった＝変更なし。
...
- 2026-09-15 Android 3.554.0 を同期（人員過剰(covO)の退避研磨 `V6HotfixPasses.CovORelief.cs`＝Kotlin `CovOReliefPolish`）:
  ...
  `CovOReliefEarly`（HF66 直後にも退避、既定 OFF＝Android tools/loop で早期配置は後続パスの経路を変え 8 勝 9 敗の揺れ）。
...
- 2026-09-10 配色がOSのダーク/ライト設定に追従してしまっていた（ユーザー実機報告「配色などが見にくい」、
  ...
  「OSのテーマへ追従」が働いていた（`RequestedTheme`/`ElementTheme`/`ActualTheme` の使用箇所はアプリ全体で
  ゼロだったことを grep で確認）。Android本家は `MainActivity.kt:43` で `MagiTheme(3)`＝UD高コントラスト
  配色を**常に固定**で呼び出しシステム設定を一切見ない（D8「外観はUD固定」）。WinUI側に同じ強制が
  無かったため、OSがダークモードのユーザー機では、`MagiTheme.xaml` で明示的にブランド色を上書きした
  要素（白背景・黒文字の高コントラスト固定）と、上書きしていない既定コントロール（ボタン/スクロールバー
  等のchrome）がダーク化される既定WinUI3スタイルとが混在し、一貫性のない配色になっていたと考えられる。
  `MainWindow.xaml` のルート `Grid` へ `RequestedTheme="Light"` を明示し、Android と同じ「システム設定に
  関わらず常に同じ配色」を強制（`ContentDialog` 等は `XamlRoot` 経由でこのルートの配下になるため連動する）。
  サンドボックスでは実機検証不可（前回コミット同様）。XAMLの整形式（XML妥当性）のみ確認済み。
...
- 2026-09-10 `windows-installer.yml`のバージョン自動採番（ユーザー指示「自動でバージョン上がるように」）:
  ...
  `win-v1.0.0`→`win-v1.0.1`で2回踏んだ）。マニフェスト由来のバージョン（明示入力・タグ由来は対象外＝
  尊重してそのまま使う）が既に公開済みなら、`gh release view`で空いているパッチ番号を見つかるまで
  自動で繰り上げる。マニフェスト自体は編集不要になった。
...
- 2026-09-06 Android 3.505.4 と同期（ループ Iteration 4 第一弾）: `ViolationComponentRepair` に起点からの候補生成（半径 1: セル違反の別シフト変更と
  ...
  （`PostOptimizationParams.ComponentRepairFinal`）。テスト +1。MagiEngine.Tests 778 OK。
