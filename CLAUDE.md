...
## Kotlin と C++ の棲み分けを1文書へ（3.367.0, ユーザー指示「KotlinとC++の棲み分けする」）

境界の根拠が CLAUDE.md の「ネイティブ加速」各節（第1〜3期・Stage1〜13）に散らばっており、
**「新しいコードをどちらに置くか」を1箇所で答えられる文書が無かった**（`docs/v6_engine_native_port.md` は
見出しを確認したところ **Web→Kotlin** の移植を扱っていて C++ 側ではない）。`docs/kotlin_cpp_split.md` を
新設し README 目次へ追加。**コード変更なし**（docs＋README＋versionName のみ）。

- **判断規則を1問に圧縮**: 「そのコードは `UnifiedViolationChecker` の `weightedScore`/`breakdown` を読むか」。
  読むなら例外なく Kotlin（チェッカーが正＝番兵の前提であり、複製すれば第2の意味論ができて必ずドリフトする）。
  読まない＋反復が百万回オーダーなら C++ 候補（ただし2層番兵必須）。読まないが数回〜数千回なら Kotlin。
- **書く前に全部数え直した**（HF77＝コメント≠実装の再発防止）: JNI 境界は `NativeBridge.kt` の
  `external fun` **17個**（ABI_VERSION=7）／C++ は `magi_native.cpp` **2,285行**の1ファイルのみで、
  Kotlin の `v6` は **21,932行**＝C++ はエンジンの約10%／`UnifiedViolationChecker.check` の Kotlin 呼出
  **167箇所**に対し C++ の `weightedScore`/`breakdown` は **0箇所**／研磨パスは `V6HotfixPasses` に **21本**＋
  独立ファイル4本／診断5種は Kotlin のみ（C++ 実装0）。
- **番兵の内訳も数え直して訂正**: 当初「21箇所」と書きかけたが、その grep は `TuningTelemetry.parityChecks`
  （計数カウンタ）と宣言を含んでいた。実際の `NativeGate.disable(...)` 発火点は **11箇所**＝
  自己整合4（SA/LAHC/ALNS/研磨）＋Kotlin照合5（同4チャンク＋起動時フル評価）＋ハンドル生成失敗2。
  `parityCheckEnabled` が切るのはチャンク側4箇所だけで、自己整合とハンドル生成検査は常時 ON。
- **移植しないと決めた項目を表で固定**（再提案しない）: チェッカー本体（実測5〜7%に対し
  `ViolationReport` のマップ束の JNI 整列が相殺）／`V6LateOperators`（Stage12）・後処理チェーン（Stage13）
  ＝300秒予算の約0.5%かつ採否ゲートが `breakdown` 依存（3.153.0）／21+4本の研磨パス／5つの診断／
  `fullEvalParts` のビット化（番兵のオラクルなので速いほうを正にしない）。
- **未測定の選択肢も正直に分離**: Kotlin 側 `DeltaEvaluator` のビット化は同じ2倍が取れる**見込み**だが
  測っていない、既定経路はネイティブなので効くのは番兵発火後の退化経路だけ、と明記。
- あわせて README の「最終更新」が 3.306.0 のまま stale だったのを 3.360.0〜3.367.0 の要約へ更新
  （自分で定めた「目次が古いと信頼が崩れる」ルールに反していた状態の解消）。

## 差分評価の速度は「言語」でなく「ビット化」で決まると実測（3.366.0, ユーザー質問「Kotlin/C++/C# のどれが速いか」）

同一実データ（golden_state）・同一マシン・同一の手の作り方（ランダム i,j ＋その職員の担当可シフト）で
20M手 × 各3回、`nice -n 19`。C++ 側に `--bench-real`（opt-in・CI の既定呼び出しには影響しない）を追加して
Kotlin `DeltaEvaluator.apply` と直接比較した。

| 実装 | 中央値 | 全数 |
|---|---|---|
| C++ scalar | 0.60 M手/s | 0.60 / 0.59 / 0.60 |
| **Kotlin** `DeltaEvaluator.apply` | **0.58 M手/s** | 0.58 / 0.58 / 0.58 |
| C++ bit-op | 1.17 M手/s | 1.25 / 1.15 / 1.17 |

**Kotlin と C++ scalar はほぼ同等（差3%）。C++ が2倍速いのは 3.172.0/3.174.0 の bitset＋popcount による**
（`rowMask`/`dayShiftMask` で c1窓・c41/c42・c3窓マッチを O(1) 化）。Kotlin 側にはビット化が**1つも無い**
（`bitCount`/`rowMask`/`dayShiftMask` の grep が 0件）ので、scalar 同士の比較になっている。
両者とも `previewMove`＋`commit`＋`score` の同じ仕事をする（`apply` の本体で確認）。

**C# は測っていない**（この環境に `dotnet`/`mono`/`csc` が無い）。推測で順位は言わない。Android アプリなので
そもそも選択肢ではない。

**含意（推測と明示する）**: `java.lang.Long.bitCount` は JIT が POPCNT へ落とすので、同じビット化を
Kotlin でやれば同じ2倍が取れる見込みが高い。逆に第3期ネイティブ移行の実質的な利得は「C++ にしたこと」より
「ビット化＋番兵つきチャンク化」だった、という読み替えになる。**測っていないので見込み**。

**限界**: x86-64 ホストでの数字。実機は arm64 で ART は HotSpot C2 より最適化が弱いため、**実機では C++ 有利へ
振れるはず**（これも推測）。JNI 往復は両者とも含まない（実アプリでは C++ はチャンク＝数千手/呼で償却される側）。

## /code-review 指摘5件の修正＝ログの実態との食い違いと無駄計算（3.365.0）

3.361.0〜3.363.0 に対する `/code-review` の指摘を全件コード照合し、**5件とも実在**を確認して修正した。
勤務表・採否・スコアは一切不変（ログの整合と無駄計算の除去のみ）。

- **[表示の矛盾・最も重い] Watchdog の実効閾値がログ側で独立に判定されていた**。`effectiveStallMs` と
  別の条件式を書いていたため ①covU 壁で短閾値になった実行でも「通常=長270s」と出て**直下の EarlyStop
  （停滞37s無改善で停止）と矛盾** ②3.361.0 で `wall` から covU 条件を外した結果 `c3nWallResult` が
  covU 床超えの局面でも true になり得るようになり、**実際は長閾値なのに「c3n壁=短」と誤表示**。
  述語を `nonCovUSettled`/`covUSettled` として切り出し、`effectiveStallMs` とログの**両方がそれを読む**形へ。
  以後この2つがずれることは構造的に起きない。パラメータ空間の総当たり＋c3n壁単独では短くならないことを
  テストで固定（**壊すと4件落ちる**ことを実行で確認）。
- **[誤誘導] EarlyStop の壁注記が陳腐化キャッシュを素で読んでいた**。診断は停滞ゲートが開いたときしか
  再評価されないため、covU が0になり groupViol だけ残った局面で古い true が残り「希望を1件調整するか
  担当を追加」と案内し得た。注記を**最終値でその側が実際に壁を根拠にしたか**で判定する形へ
  （その状況では自動的に偽になる）。c3n 側も同型に修正。
- **[無駄計算] `diagnoseCoverage` の `report` 引数が本体で一度も読まれていなかった**。既定値
  `UnifiedViolationChecker.check(state, schedule)` なので、**省略した呼び出しは毎回フル checker
  (19族×S×T) を計算しては捨てていた**。省略していたのは停滞判定の遅延診断＝全ワーカーが同時に入りうる経路。
  引数ごと削除（呼出は VM の1箇所のみ）。
- **[雷鳴群れ] 遅延診断の版キャッシュを計算後に書いていた**。停滞ゲートが開いた瞬間に全ワーカーが同時に
  未キャッシュ枝へ入り、各自がフル診断（checker＋`findCovUChain` BFS）を回して結果を捨てていた。
  **版を計算前に CAS で確保**する形へ（c3n/covU 両方）。負けた側は直前世代の値を読むが、この判定は
  停滞閾値の選択にしか効かず次のポーリングで最新を読む＝元から世代遅れを許容している設計。
- **[観測性] covU 壁が非発火時に完全に不可視だった**。Watchdog に `人員不足の壁診断=OFF/未実行/判定結果` を
  追加し、TimeBudget 行の頭打ち条件にも covU 壁を明記（3.283.0 の「発火しなかった理由をログだけで追う」目的）。
- **[docs] KDoc の最終段落が2回貼られていた**＋`[3.361.0/検証中]` マーカーが 3.362.0 の既定ON確定後も残存。
...
内部フラグのみ）。3.298.0/3.304.0 と同じ3層で配線した。

- `UiState.covUWallEarlyStop`（既定 true＝`PolishGate` の既定と一致）／`MagiViewModel.setCovUWallEarlyStop`
  （ゲートと UiState を同時更新＋操作ログ）／設定タブ→詳細設定の「人員不足が詰んだときの切り上げ」。
- **文言は実測値で書く**。ON=「5分の設定が約1分40秒で終わりました。まれに、最後まで回せば見つかった
  はずの必須違反1件を取り逃します」／OFF=「決めた時間を最後まで使い切ります」。
  **どちらの状態も危険ではないので警告色は使わない**（他の2トグルは ON が非推奨なので警告色＝意味が違う）。
- `TuningTelemetry.covUWallHits` を追加し「設定の効き」行へ。ON なのに一度も壁と判定しない実行が続けば、
  そのデータでは効いていないと数字で言える（3.356.0 の目的と同じ）。

### 併せて直した既存の穴＝調整トグルの起動時同期
調整トグルの実体はプロセス全域の `NativeGate`/`PolishGate` で、UiState はその写し。**ViewModel が
作り直されると UiState だけ既定へ戻り engine 側は前の選択を保持する**＝画面が嘘をつく
（例: OFF にしたのに再生成後は ON と表示され、実際は OFF のまま）。`init` で gate → UiState の向きに
6つまとめて写す（gate が実際に効いている設定なので向きはこちらが正しい）。通常の起動＝新規プロセスでは
gate も既定なので値は変わらない。**この穴は covU 壁に限らず既存4トグル全部にあった**。

## covU 壁の早期終了を既定 ON（3.362.0, ユーザー指示「covUWall=ONデフォルト」）

3.361.0 の A/B を完了し、指示により既定を ON にした。**測定結果を曲げずに記録する。**

### 実測（PORTFOLIO・workers=4・real/user 各3反復）
| 予算 | 時間（中央） | 早期終了 | 判定 |
|---|---|---|---|
| 60s | real 60.0→59.7s / user 60.0→56.1s | 0/6 → 5/6 | 短縮 最大 6.5%＝**事前登録の 10% に未達** |
| 300s | real 298.7→**104.5s** / user 298.7→**93.9s** | 0/6 → **6/6** | **65.0% / 68.6% 短縮** |

**60s で測ったのは設計の誤り**だった。ウォッチドッグの閾値は全て予算に比例し（`stallMs`=予算9/10、
`stallHardMs`=予算/8、`minRun`=予算/6）、かつ AUTO は **211s 以上でしか PORTFOLIO を選ばない**＝
60s は利用者が PORTFOLIO で踏まない帯。**基準は一切変えず予算だけを実運用帯へ変えて再測**した
（結果を見てから基準を緩めたのではない）。

### 品質の代償（事前登録の基準は満たしていない）
- hard の中央値は両者 4 で不変。
- ただし **hard=4 の中で weighted 中央値は ON が real +206 / user +53 とわずかに悪い**。
- さらに **real は OFF が3回中1回 hard=3 に到達（ON は 0/3）**。早く止めれば、まれに終盤で見つかる
  HARD 改善を捨てる。

よって私が事前登録した「品質が悪化しない」は**満たしていない**。それでも既定 ON にするのは、
待ち時間を約1/3にする価値がその代償を上回るという**ユーザーの判断**（3.341.1 と同型の
「品質と電池・待ち時間の交換＝業務判断」）。

### 併せて入れたもの
`EarlyStop` ログに covU 壁の理由を明示（「残る必須=人員不足は CoverageDiag が『いまの希望・担当の
ままでは減らせない』と判定済み。希望を1件調整するか担当を追加すると先へ進めます」）。
**予算300秒の指定で約90秒で返ると不具合に見える**ため、止めた理由を必ず1行残す。

### 既知の限界
- **利用者が OFF に戻す手段が現状ない**（`PolishGate` の内部フラグのみで UI 未配線）。必要なら
  3.298.0 と同型で詳細設定へ足せる。ただしオプションを減らす方針とは逆方向。
- n=3。PORTFOLIO の run 間ばらつきは大きく、品質側の差（+206/+53）が実在の劣化か揺れかは
  この標本数では分離できない。時間側（65〜69%）は揺れでは説明できない大きさ。

## 設定トグルの一括 A/B（3.361.0 の副産物・判定不能と記録）

60s × 3反復 × 3データ（54 run）で全 Kotlin 側トグルを1つずつ反転。**covUWall 以外はノイズから
分離できなかった**。方法論の問題を2つ露呈したので、そちらを記録する:

1. **weighted の中央値比較は hard を跨ぐと成立しない**。帯を膨らませた外れ値（real BASE 5882、
   user wideC3n 6173）は測定ノイズではなく **HARD が1つ少ない解に着地した run**（hard=3・covU=2）。
   weighted は covU=8000 支配なので約6000ずれる。**hard で層別してから比較する**のが正しい。
2. **層別すると今度は n が足りない**。real の BASE は hard=4 が2件しか残らず帯が 24 に潰れ、
   その結果**全条件が「改善」**と出る（3.306.0 で効果なしと実測済みの escape まで）＝人工物。
   **n=3 では帯そのものがノイズ**（同一データで 24〜397 と振れる）。

方向だけ記録（判定はしない）: 事前フィルタ 混在／崩し範囲 2/3 負／立て直し方 3/3 負(小)／
仕上げ最適化 **3/3 負・c1 も 3/3 減**（108→96, 74→74, 72→57）。n=3 では符号検定でも最良 p=0.125＝
有意に届かない。**「差が出なかった」を「効果がない」と言い換えない**（教訓#11）。

**ネイティブ加速と Kotlin照合はこの環境では A/B 不能**（ホストJVM は arm64 の `.so` を読めず
`NativeBridge.available=false` 固定）。実機ログでしか判定できない。

## covU 壁を停滞判定へ（既定OFF・A/B 測定中）＋遅延ゲートの取り残し修正（3.361.0）

ユーザー指示「あなたの環境で AB 検証してください」。保留3件のうち **`allBlockedNow` → ウォッチドッグ配線**を
選んだ（効果が「時間」で現れるので、品質差がノイズに埋もれる他2件と違い測れる＝3.310.1 の失敗を繰り返さない）。

### 何が足りなかったか
`effectiveStallMs` の短い閾値は covU が**静的な**構造下限（担当者数ベースの `structuralHardFloor`）に
達したときしか効かない。実データの covU は希望固定・禁止連続で**動的に**塞がるため（3.344.0 で
CoverageDiag が「充足可能3枠だが今は不能」と実測）、`bestHard > hardFloor` のままとなり長い閾値
（予算9/10）が選ばれ、実質的に発火しない。

### 実装（`PolishGate.covUWallEarlyStop`・**既定 OFF**）
- `effectiveStallMs` を **covU 側と非covU 側の独立判定**へ分解:
  `nonCovUSettled = nonCovU==0 || (c3nのみ && c3n壁が証明済み)` ／
  `covUSettled = (hard − nonCovU) <= hardFloor || covU壁が証明済み`。
  旧実装は covU の条件（`bestHard <= hardFloor + nonCovU`）を **c3n 分岐の中に**書いていた（同値だが所属が誤り）。
- covU 壁の遅延証明は c3n 壁と同型（best 世代ごと1回・停滞が stallHardMs 超のときだけ・~20ms）。
- **既定 OFF のとき旧式と厳密に一致することを、パラメータ空間の総当たり（640通り）でテスト固定**。

### 実測が私の配線バグを捕まえた（推測で終わらせなかった結果）
最初の A/B で B が一度も発火しなかった。原因を推測せず直接測ったところ、real/user とも
**covU 側（不足3枠すべて `blockedNow`）も非covU 側（c3nのみ・`ForbiddenDiag.allBlocked=true`）も
壁は証明済み**だった。発火しなかったのは `shouldStop` 内の遅延ゲートに、`effectiveStallMs` から
外したはずの covU 条件が残っていて **`c3nWallProven()` が一度も呼ばれていなかった**から＝私のリファクタの
取り残し。推測で書いていたら「c3n が証明されないから inert」という**逆の結論**を報告していた。
ゲートを非covU 側の条件だけに直した（covUWallEarlyStop=false のときの判定結果は従来と厳密に同一。
`c3nWallProven()` が呼ばれ得る局面は増えるが、停滞時のみ・世代ごと1回キャッシュ・~20ms）。

### A/B の設計（基準は測る前に固定した）
採用は**両方**満たすときのみ: ①品質＝3データセットとも B の weighted 中央値が A を上回らず hard も悪化しない
②時間＝少なくとも1データセットで壁時計中央値が A より 10% 以上短い。欠ければ**不採用**でコードを残さず記録のみ
（3.307.0 の規律）。**測定は本コミット時点で進行中**＝既定 OFF のまま出す。

### 予備観測（120s・1反復）で確認した注意点
golden で A=2513 / B=2233 と 280 の差が出たが、**golden は hard=0 に到達するので covU 壁の判定は元から効かない**
＝この差は純粋な run 間ばらつき（3.310.1 の「PORTFOLIO の分散 200〜300」と一致）。よって本番の A/B は
**同一条件 BASE の反復幅を「帯」として先に測り、その内側なら差なしと判定する**設計にした。
