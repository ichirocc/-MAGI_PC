...
## 対応方針待ちだった4項目を実測に基づき解消（3.367.0）
ユーザー指示「賢く深く考え対応する」を受け、3.366.0で「設計トレードオフのため未対応」としていた4項目を
1つずつ実測してから対応方針を決定・実施した。**探索・重み・スコアは無関係（test/tools/CIのみ）**。
- **①常時実行 vs `--shared-only`限定**: 「削るか残すか」の二択でなく**両方を安く両立**させる方針に。
  TSANビルドのコストを実測（`-O1 -g -fsanitize=thread`）したところ**ビルド~7秒＋実行~0.05秒＝合計10秒未満**
  （コード内の既存コメント「10〜20倍遅い」は重い並列パリティループの話で、この軽い検査自体には
  当てはまらないと判明）。この安さを根拠に**専用のTSAN CIジョブを新設**（`shared-handle-tsan`、
  `.github/workflows/native-parity.yml`）し、コメントが元々主張していた「TSANでビルドすれば検出される」を
  「手元で覚えていれば手動で検証できる」から「CIが毎回自動で検証する」へ格上げした。**その代わり**、
  通常ビルド版（`native-parity`ジョブ）の`runSharedHandleConcurrency`呼出は**flatフィクスチャごとの
  重複を解消し1回に限定**（`MagiProblem`が読み取り専用で共有安全かという盤面データに依らない構造的性質の
  検査のため、2件目以降は同じ検査を別データで繰り返すだけの無駄と判明済み＝3.366.0のEfficiency角度指摘②の解消）。
- **②`--shared-only`+`--expect`同時指定**: 無警告スキップをやめ`note: --shared-only ignores --expect=...`
  を出すよう変更。挙動（`--expect`を使わない）自体は変えない＝罠を可視化するだけ。
- **③exit code重複**: 実測でなく設計精査。現状は既に**1=検査が実際に走って不一致を検出／2=検査自体が
  実行できなかった（setup/usage問題）**という一貫した二層構造で、この分離こそが呼出元（CI/スクリプト）に
  とって本当に必要な情報。「2」の中の細分（loadFlat失敗／`--shared-only`にflatなし／`--expect`件数不一致）は
  現状どのスクリプトも判別しておらず、指摘元自身も「Low severity」と評価済み。過剰な分割はせず、
  ファイル冒頭に**exit code契約をコメントで明記**するに留めた（意味は変えず読み手の理解だけ揃える）。
- **④簡素化系4件**: 全て採用。比較ループを`out[0]`(status)チェックと共存する単一のbool式へ／スレッド数
  `8`を`kSharedHandleThreads`定数へ（呼出は1箇所のまま）／`oneChunk`のラムダキャプチャを`[&p,&board]`へ
  絞り込み（この関数自体が「将来のローカル変数が誤って多スレッドクロージャに巻き込まれる」class のバグを
  検出する目的のため、自身の捕捉も最小化するのが一貫性がある）／`flatIdx++`の分岐重複を1箇所へ集約。
- **検証**: 5シナリオ（`--shared-only`引数なし/1flat/`--expect`併用/件数不一致/不正パス）を全て実行し
  期待どおりの終了コード・メッセージを確認。CI相当のフル実行（golden.flat+sample_v6.flat、`--expect`2件）を
  複数回再実行し、`SHARED-HANDLE`行が1回に減ったこと以外は完全に同一結果（両方MATCH・4,195,533手・
  0 mismatches）であることを確認。TSANビルドでも競合警告なし。新設したCIジョブの手順（ビルド→フィクスチャ
  生成→`--shared-only`実行）もサンドボックスで一字一句再現し成功を確認してから配線。
