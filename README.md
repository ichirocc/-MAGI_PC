...
**最終更新**：2026-08-22（3.416.0 ユーザー方針「**休は通常のシフト定義**」を main 全体へ適用——評価層（Evaluator/Δ/checker/C++）は 3.345.0 以降 restIdx 参照ゼロ＝既に統一済みと監査で確定し、残っていた編集ガード2つ（3.106.0 の削除禁止・3.415.0 の改名禁止=R-04）を撤廃。削除の fill 式は「削除後の一覧で解決した既定シフト」へ差し替え（旧式は休自身の削除で範囲外セルを作る欠陥があり、ガードがそれを隠していた）。fill/park の既定値（3.345.0 合意）と診断の restCapacity 系・表示の休セル淡色化は維持。3.415.0 外部レビュー撤回文書のR-01〜R-05を照合。R-01/R-02は3.411.0で・R-03は3.309.0のunresolvedRows案内で・R-05は3.327.0/3.328.0の検査2hで既に対応済みと確認。**R-04（休シフトの記号改名に入口ガードが無い）だけが実在**——`removeShift`は3.106.0で休の**削除**を禁止するのに**改名**にはガードが無く、`restShiftIndex`の記号一致解決が黙って先頭シフトへ倒れうる状態だった。`ws1EditShift`へ削除と同じ姿勢の入口ブロックを追加。100件レビュー自体は先行表を「不具合として不正確」と自ら撤回する結果になった。3.412.0 100件レビューの照合を **56件まで**進め、実在した項目を順に修正。3.411.0＝
