...
**最終更新**：2026-08-23（3.428.0＝**100件レビューの未確認70件を全部当てて実在4件を修正**。①`RunFiles.clear` の消し残りを読んでいたのは1箇所だけで残り9箇所は捨てていた（3.410.0/B-06 の契約の取り残し）＝停止や失敗が次回起動で「中断＝再開できます」と誤案内されるのに痕跡が残らない ②前景サービス化の失敗が無言 ③原子置換を諦めたことが残らない ④`hf66DataHardening` だけ埋めシフト規則が `fillShiftIndex` の写しのまま（休が先頭でないデータで勤務シフトへ倒れる。実データでは no-op と測ってから委譲）。**実装中に既存テストが危険を捕まえた**＝`onNonAtomic` を `commitGuard` の後ろに足したせいで、本番2箇所の末尾ラムダ `{ ownsFiles() }` が束縛先を変え **3.385.0 の TOCTOU 再確認が既定 `{true}` に落ちていた**。末尾R表4件は全部すでに閉じており表自体が stale。539テスト green。 / 3.427.0＝ペア禁止の行を「吉の休 ✕ 古の休」の読める形にして行下の
