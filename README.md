...
**最終更新**：2026-08-23（3.434.0＝職員削除の確認ダイアログに「勤務・希望も消えます」警告が無いドアが
あった問題を修正。同じ削除操作(vm.ws1RemoveStaff)なのに年間マスタードアと職員管理ドアで警告内容が違って
いた。自律監査（design-review の観点を手動適用）の継続で発見。3.433.0＝シフト/グループ削除ダイアログの
参照件数開示をスキルグループと同じ「独立した文」の形へ統一＋開発者向け内部語「インデックス」の言い換え。
