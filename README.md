...
**最終更新**：2026-08-19（3.408.0 実機ログの報告「前回実行の『グローバル最良更新』と直近実行の『更新0回』が
**同一実行の矛盾**に見える」を行単位で裏取り。**報告どおり**＝ログには実行が2回あり（16:09/16:14）、
操作ログは**履歴**・診断ログは**直近1回**なのに実行IDが無く連結されていた。`globalImproves` 自体は正しい
ことを先に実測（ホストJVMで「メッセージ3回==サマリ3回」）してから、`OpLogEntry` に実行の通し番号を持たせ
行頭 `#N`・診断セクションに `実行#N` を明示した（表示のみ・スコア不変）。
**さらに重い実バグを同じログから確定**＝`停滞274s・実効閾値37s・発火=なし・未発火の理由=現フェーズ猶予未達
(実測0s/7s)`。並列8ワーカーが1本のフェーズ名を共有するため猶予が**恒久的な拒否権**になっており、
275秒まるごと無改善でも早期終了しない。猶予を**遅延**へ降格し、無改善が閾値の2倍で必ず発火するようにした
（3.375.2 で「業務判断」として保留していた項目を、ユーザーの明示指示で実施。代償は 3.341.1 の実測＝
早期終了を外すと weighted 中央 −3.5%・p≈0.075）。／3.407.0 **同梱の見本データ `assets/sample_state_v6.json` が二重エンコードの文字化けのまま
出荷されていた**のを発見・修復。「休み」「Dﾃ」「古泉 健一」がいずれも
`U+00C0-U+00FF` と `U+0080-U+00BF` が交互に並ぶラテン文字列へ化けており（＝UTF-8 を Latin-1 として読んだ内容を
再び UTF-8 で保存した状態）、
シフト記号も職員名も全部壊れていた。アプリは `MojibakeRepair` で**表示だけは直していた**ぶん、
**自分の同梱ファイルを開くたびに「文字化けを自動修復しました」と警告**し続けていた（しかもその助言
「保存し直せば消えます」は asset には効かない）。修復は**可逆であることを確認**してから適用し、
**評価は完全に不変**（checker/Evaluator/19族 breakdown・`state_to_flat.py` の出力・後処理研磨の結果まで一致）。
再発防止に `design_lint` **P7（二重エンコード検出）** を新設し、**`design-lint.yml` を新設して P5/P6/P7 を CI へ**
（P7 が守るのは出荷される asset＝テストのクラスパスに載らないので lint 側でしか守れない）。
あわせて **500テストの断捨離**＝`EliteIntegrationRandomSafetyTest` は `@Test` の無い `main()` ハーネスなのに
名前が Test で終わるため JUnit が毎回 `initializationError` で失敗しており、**手で起動しない限り3つの安全
不変条件は一度も検証されていなかった**。実測4.4秒＝実テストへ移し、**500テスト・失敗0**に。／3.406.0 実機ログ(3.402.0/Pixel 10 Pro XL)と受領した不具合一覧を**1件ずつ実コードで裏取り**して6件を修正。
**①covO 診断が「最適化が未到達＝『直し方を探す』で解消可」と断言**していた——その真上のコメント自身が
「動かせるのに動いていないことの説明にはならない」と書いているのに。**同じ目的関数(betterReport)で実際に1手試してから**
言う形にし、負けるなら主因の族を出す（実機ログでは covO 焦点の修復が275秒走ってなお8件が残り、断言が実測に裏切られていた）。
**②「上限の合計 < 必要数」で covU を不可避と断定**していた——個人上限は SOFT(high)なので超過でき、実機ログが
covU=0・high=6（＝ちょうど 30−24）で反証。証明できる **covU+high ≥ 差** の形へ。**③messageIsError が通常メッセージへ残留**
（3.400.0 で私が作り込み。`copy` は既定値でなく**現在値**を引き継ぐ）——56箇所を明示化し、`design_lint` に **P6** を新設して
機械で止める。**④背景計算の開始でマーカー/入力の保存失敗を握り潰して Work を投入**＝無言の失敗。**⑤壊れた完了結果を
解析する前に入力・途中最良を削除**＝復元手段まで失う。**⑥Worker の `catch(Exception)` が `Error`(OOM)を拾わず**失敗通知と
後片付けが走らない。／3.405.0 監査 high の最後の1件＝**シートが守れない約束をしていた**。
勤務表のセルはいつでもタップでき、開いたシートは「タップで割当を即変更。」と言い切ってから拒否していた
（3.328.0 のガードは正しく働くが、**形が約束したことを守れていない**）。**変えられない状態ならシートを
開かない**へ——判定と文言は `MagiViewModel.editBlockedNow()` の1箇所に置き、`setCell` 系4つと
**同じ文字列**を読む（片方だけ直って食い違うのを防ぐ）。開かなければ約束は嘘にならないので、
シートの文言は条件分岐なしのまま正しくなる。3.399.0 の Snackbar に理由（何の実行中か）が出る。
／3.404.0 **「盤面を丸ごと差し替えるジョブ」の旗を1つに揃える**。
`optimizeActive` という名前が「最適化」としか読めなかったため、**同じ性質を持つ読み込み・CSV取込・
初期解生成の3つが旗を立て忘れていた**——`running=true` で画面は全ロックなのに `optimizeInFlight()` は
false のままで、**ガード側だけ全開**という逆転。初期解生成の最中にセルを編集すると `setCell` のガードを
素通りして書き込まれ、完了時の `currentSchedule = res.schedule.copy2D()` が**それを無言で上書きする**
（3.161.0 が最適化について塞いだ穴の、他3ジョブぶんの取り残し）。旗を `boardJobLabel: String?` に改名して
意味を名前に書き、5つのジョブすべてが `beginBoardJob`/`endBoardJob`（`checkSeq` と同じ通し番号で
**自分が立てた旗のときだけ下ろす**）を通るようにした。あわせて **4つある編集入口のうち
`applyStructureWithMessage(Ws1Result)` だけガードが無かった**（通るのは apt全リセットと職員一覧CSV取込＝
後者は勤務表ごと差し替える）／**`importCsv` に入口ガードが無く、走行中の最適化の `job` 参照を
キャンセルせず上書き**していた（＝「やめる」で止められないゾンビ。3.271.0 と同型の取り残し）／
停止を「失敗」と呼んでいた3経路を `CancellationException` で分離／**途中経過の盤面を完了時に消していなかった**
（あとで編集して違反チェックが走ると前の実行の古い盤面が現在のものとして出た）を同時に修正。
画面のメッセージも「計算中は…」一律から**何の実行中かを名前で言う**形へ。／3.403.0 **下限>上限を、あとから叱るのでなく入力時に止める**。
`V6SanityPort` は群のレンジ（3.399.0）も個人の回数も「下限$lo > 上限$hi で矛盾しています」と診断し
ワンタップ修正まで出すのに、**その値を入れる4つのダイアログはどれも素通しで確定できた**——直した直後に
また同じ入力ができ、しかも群のレンジの lo>hi は engine の `z < l || z > u` で**どの人数でも必ず違反**＝
期間の全日が違反になる。判定を `V6SanityPort.rangeOrderConflict(lo, hi)` として切り出し、
**事後診断2箇所と入力ダイアログ4箇所が同じ関数を読む**（片方だけ緩いと入力を通してしまうため）。
画面では確定ボタンを押せなくし、枠を赤くし、「下限は上限以下にしてください」を出す＝
3.186.0 の必要人数パネルと同じ形。両方が数値のときだけ矛盾と見なすので、空欄（未設定）・片側だけ・
下限==上限（厳密ピン）は従来どおり保存できる。ホストJVM **497テスト green**、
述語を壊すと新テストと 3.399.0 のテストが実際に落ちることを確認済み。／3.402.0 Nielsen 監査の確定項目から、他の作業と独立に閉じられる2件。
**①「直し方を探す」の最中だけ「やめる」が出ない**——`stop()` は元から `running || fixSearching` の両方を見て
戻す（3.284.0）のに、**下部バーのボタンのゲートだけ `ui.running` に限定**されていたため、探索中は止める手段が
画面上に一つも無かった（探索は `viewModelScope` で走り、外から中断する術がユーザーに無い）。ゲートを
`stop()` と同じ条件に揃える＝**判定を2箇所に書かない**。**②分析タブの日別行だけ押せない**——人別行は
`onClick = if (ac > 0) …` で修復フローへ飛ぶのに、日別行は `onClick = null` が無条件に渡され、
同じカードの中で押せる行と押せない行が混在していた。行き先は要確認一覧が既に持っている
「勤務表の該当日へ」と**同じラムダ**を使い、末尾の文字も同じ規約（日＝「勤務表→」／人＝「直し方→」）に合わせる。
表示・導線のみ＝重み・採否・エンジンは不変。／3.401.0 Nielsen 監査 high の残り1件＝**「なおすのを手伝って」が、アプリ自身の
