...
**最終更新**：2026-08-24（3.435.0＝勤務表づくり完了メッセージが生の内部識別子（"optimize:PORTFOLIO"等）を
画面に出していた（3.400.0が別経路で既に除去した同型漏れの取りこぼし）＋開始/完了メッセージの語彙不一致
（最適化/ソフト研磨→整え）＋仕上げ最適化の失敗メッセージが生の例外文を出していた、の3件を修正。自律監査
（design-review の観点を手動適用）の継続で発見。3.434.0＝職員削除の確認ダイアログに「勤務・希望も消えます」
