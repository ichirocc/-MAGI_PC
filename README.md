...
**最終更新**：2026-08-23（3.433.0＝シフト/グループ削除確認ダイアログの参照件数開示をスキルグループと
同じ「独立した文」の形へ統一＋開発者向け内部語「インデックス」の言い換え。自律監査（design-reviewの
観点を手動適用）で発見。ついでに外部提示の native/JNI 層レビュー2本を実コードで裏取りし、いずれも
コード変更不要（実装済み・事実誤認）と確認。表示のみ・エンジン/採点は不変。design_lint exit=0。
